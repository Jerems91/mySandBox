...
public class TrtQtePlus implements ITraitement {
	
	private static final Logger logger = LogManager.getLogger(TrtQtePlus.class);
	...
	public void routeRequete(HttpServletRequest request, HttpServletResponse response, String vue) throws IOException, ServletException {
		...
		// Récupération du code produit de l'achat dont il faut augmenter la quantité
		int codeAchatPlus = CtrlUtils.getCodeProduitFromRequest(request,CtrlUtils.CODE_ACHAT);
		...
		// Récupération du code produit sous forme d'entier réussie pour l'achat concerné
			...
			// Augmentation de la quantité de l'achat dans le panier
			CtrlUtils.quantitePlus(monPanier, String.valueOf(codeAchatPlus));
